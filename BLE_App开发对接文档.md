...
**最小可用流程**：扫描 `C3_BLE_01` → 连接 → 发现服务 → 对 `F001` 写 `31` → GPIO3 输出 3.3V，负载导通。
...
| 字段 | 值 | 说明 |
|---|---|---|
| 设备名 | `C3_BLE_01` | 已写入广播包，扫描阶段即可直接读取，**推荐按名称过滤** |
...
> 请一律使用 **设备名 `C3_BLE_01`** 或 **服务 UUID `0xF000`** 进行扫描过滤与重连匹配。
...
└──────┬──────┘                                    └──────┬───────┘
       │                                                  │
       │  ① 扫描（过滤 name=C3_BLE_01）                    │
       │ ──────────────────────────────────────────────►  │
       │                                                  │  持续广播（30-60ms）
       │  ◄────── ADV_IND (含设备名 C3_BLE_01) ───────────│
       │                                                  │
       │  ② connectGatt()                                 │
       │ ──────────────────────────────────────────────►  │
       │  ◄──────────── 连接完成 (conn_handle=1) ─────────│
       ...
       │  ⑥ 写 F001 = 0x31（开）                          │
       │ ──────────────────────────────────────────────►  │
       │  ◄──── Write Response (status=0 成功) ───────────│   GPIO3 → 0V（继电器吸合）
       │                                                  │
       │  ⑦ 写 F001 = 0x30（关）                          │
       │ ──────────────────────────────────────────────►  │
       │  ◄──── Write Response (status=0 成功) ───────────│   GPIO3 → 3.3V（继电器释放）
       │                                                  │
       │  ⑧ 断开                                          │
       │ ──────────────────────────────────────────────►  │
       │                                                  │   自动重新开始广播
...
**① 扫描**
- 按设备名 `C3_BLE_01` 过滤最省事；也可按服务 UUID `0xF000` 过滤（更严谨）。
...
| 操作 | 返回值 | 说明 |
|---|---|---|
| Read `0xF001` | ASCII `hello`（5 字节，`68 65 6C 6C 6F`） | 固化的连通性验证字符串 |

> **推荐用法**：把 Read `0xF001` 当作 App 的**连通性自检 / 心跳**。
> 能读出 `hello` 即证明链路、特征、回调全部正常。
> ⚠️ 注意：Read **不返回**当前开关状态（见第 8 节已知限制）。
...
- 固件设计为**只发一次**（"连上后回发一个"），不是周期性推送。
- 若 App 需要持续状态回传，需要修改固件（见第 8 节）。
...
| 现象 | 排查顺序 |
|---|---|
| **扫描不到设备** | ① 芯片是否上电并打印 `BLE ready, name=C3_BLE_01, advertising...`；② 手机定位开关 + App 定位权限；③ 距离是否 < 1 m；④ 换 LightBlue / nRF Connect 交叉验证 |
...
| **能读到 `hello` 但写不进去** | 特征选择正确但写入数据格式异常；尝试单字节 `31` / `30` |
...
| # | 限制 | 影响 | 建议改法（固件侧） |
|---|---|---|---|
| 1 | Read `F001` 固定返回 `hello`，**不返回当前开关状态** | App 启动/重连后无法知道灯是开是关 | 修改 `gatt_svr_chr_access` 的 READ 分支，按当前电平返回 `"ON"` / `"OFF"` |
| 2 | Notify 仅在订阅当刻发一次 `connected` | 无法实现状态实时回显 | 在 `gatt_svr_chr_access` 的 WRITE 分支、`gatt_send_one` 之外，增加状态变化时主动 `ble_gatts_notify_custom` |
| 3 | 地址随机、无配对 | 任何手机都能控制 | 提高 `CONFIG_BT_NIMBLE_SM_LVL`，启用 Passkey 配对 |
| 4 | 未知指令静默（仅串口打印） | App 无法获知指令是否被拒绝 | 让未知指令走 Notify 回传错误码 |
| 5 | 单引脚单路开关 | 无法控制多路 | 扩展 GATT 特征（如 `0xF002`）或使用指令格式 `pin,level` |

> 表 1、2 是实现"App 显示真实开关状态"的必要条件。若 App 需要显示"当前是开还是关"，**必须**先改造这两项，否则 App 只能靠本地记录猜测状态（断电重启后会失准）。
...
| 设备名 | 固定 `C3_BLE_01` | 加入 MAC 后几位做唯一化，避免多设备同名 |
...
| 参数 | 值 | 来源 |
|---|---|---|
| 设备名 | `C3_BLE_01` | `main.c` → `BLE_DEVICE_NAME` |
